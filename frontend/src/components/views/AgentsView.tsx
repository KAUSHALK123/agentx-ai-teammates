...
export const AgentsView: React.FC = () => {
  ...
  const agentList = Object.values(agents || {}).length > 0 
    ? Object.values(agents) 
    : Object.values(INITIAL_AGENTS);
    ...
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-extrabold shadow-sm transition-all cursor-pointer"
        >
            ...
            >
              <div className="space-y-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-slate-900">{agent.name || 'AI Teammate'}</h3>
                      <p className="text-xs text-slate-500">{agent.title || 'Specialist'}</p>
                    </div>
                  </div>
                ...
                <p className="text-xs text-slate-600 leading-relaxed font-normal">
                  {agent.description || 'Autonomous specialist.'}
                </p>
                ...
                <div className="grid grid-cols-2 gap-2 bg-slate-50 p-3 rounded-xl border border-slate-200 text-center">
                  <div>
                    ...
                    <p className="text-sm font-extrabold text-slate-900">{(agent.tasksCompleted ?? 1240).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-500 uppercase font-mono font-bold">Success Rate</p>
                    <p className="text-sm font-extrabold text-emerald-600">{agent.successRate ?? 98}%</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {capabilitiesList.slice(0, 4).map((cap: any, i: number) => {
                ...
                <div className="flex items-center justify-between text-xs text-slate-500 pt-1">
                  <span className="flex items-center gap-1">
                    ...
                    {toolsList.length} Tools Connected
                  </span>
              ...
              <div className="pt-4 border-t border-slate-100 grid grid-cols-2 gap-3">
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold border border-slate-200 transition-colors cursor-pointer text-center"
                >
                  Open Agent
                </button>
                <button
                  onClick={() => handleStartTask(agent.role)}
                  className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-xs transition-all flex items-center justify-center gap-1 cursor-pointer"
                >
