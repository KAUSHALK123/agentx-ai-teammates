...
export const AgentDetailView: React.FC = () => {
  ...
  const agent = agents[selectedAgentRole] || agents.support || INITIAL_AGENTS[selectedAgentRole] || INITIAL_AGENTS.support;
      ...
      <div className="glass-card rounded-2xl p-6 md:p-8 border border-slate-200 relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-slate-100">
          <div>
            ...
            <p className="text-xl font-extrabold text-slate-900 mt-1">{(agent.tasksCompleted ?? 1240).toLocaleString()}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-xl font-extrabold text-indigo-600 mt-1">{agent.avgResponseTime || '1.6s'}</p>
          </div>
          <div>
        ...
        <div className="space-y-3">
          {agentTasks.length === 0 ? (
            <div className="p-8 text-center text-xs text-slate-500 font-medium bg-slate-50 rounded-xl border border-slate-200">
              No recent tasks dispatched to {agent.name} yet. Click "Start Task" to execute an autonomous business workflow.
            </div>
          ) : (
            agentTasks.map(t => (
              <div
